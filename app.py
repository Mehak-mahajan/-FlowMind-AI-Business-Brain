...
<style>
    ...
    .user-bubble, .user-bubble * { color: #000 !important; }
    .assistant-bubble {
    ...
    .assistant-bubble, .assistant-bubble * { color: #000 !important; }
    .source-tag {
        font-size: 11px;
        color: #075E54;
        margin-top: 3px;
    ...
    .lead-box, .lead-box * { color: #4A3B00 !important; }
    .alert-box {
    ...
    .alert-box, .alert-box * { color: #6B0000 !important; }
    .gap-box {
    ...
    .gap-box, .gap-box * { color: #003366 !important; }
    .welcome-box {
        text-align: center;
        padding: 40px;
        background: white;
...
def init_db():
    ...
    # Migration: real support contact — set once by the owner, inserted
    # deterministically into fallback messages (never generated by the AI,
    # so it can never be wrong or invented)
    try:
        c.execute("ALTER TABLE businesses ADD COLUMN support_email TEXT DEFAULT ''")
        c.execute("ALTER TABLE businesses ADD COLUMN support_phone TEXT DEFAULT ''")
        conn.commit()
    except sqlite3.OperationalError:
...
# =====================================================
# ---- SUPPORT CONTACT (NEW) — owner-entered, inserted deterministically
# into fallback/apology messages. Never generated by the AI, so it can
# never be wrong. ----
# =====================================================
def save_support_contact(business_id, email, phone):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE businesses SET support_email=?, support_phone=? WHERE business_id=?",
              (email, phone, business_id))
    conn.commit()
    conn.close()

def get_support_contact(business_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    row = c.execute(
        "SELECT support_email, support_phone FROM businesses WHERE business_id=?", (business_id,)
    ).fetchone()
    conn.close()
    if row:
...
def format_contact_line(business_id):
    """Deterministic contact text to append to any fallback message —
    built from real owner-entered data, never from the AI."""
    email, phone = get_support_contact(business_id)
    parts = []
    if email:
    ...
    if parts:
        return " You can also reach us directly: " + " | ".join(parts)
    return ""
...
def build_search_query(question, history):
    """
    A short follow-up like 'what about the price?' searches badly on its
    own — the knowledge base has no idea what 'it' refers to. Prepending
    the previous question ('do you have lipsticks?') gives the search
    enough context to find the right chunk, without changing what the
    AI actually answers (it still sees the original short question).
    """
    prior_user_messages = [m["content"] for m in history[:-1] if m["role"] == "user"]
...
with st.sidebar:
    ...
    if st.session_state.is_admin:
        ...
        # NEW: Support Contact — the fix for fake/invented contact details.
        # This exact text gets inserted into fallback messages, never
        # generated by the AI, so it can never be wrong.
        st.markdown("### 📞 Support Contact")
        st.caption("Your real contact info — shown automatically when the AI hands off to you. "
                   "This is never AI-generated, so it's always accurate.")
        current_email, current_phone = get_support_contact(business_id)
        col1, col2 = st.columns(2)
        with col1:
            new_email = st.text_input("Support email", value=current_email, placeholder="you@yourbusiness.com")
        with col2:
            new_phone = st.text_input("Support phone", value=current_phone, placeholder="+91 98765 43210")
        if st.button("💾 Save Support Contact"):
            save_support_contact(business_id, new_email, new_phone)
            st.success("Saved — customers will now see your real contact info in handoffs.")
...
if active_business_id and chunks:
    ...
    if question:
        ...
        # Expand short follow-ups with the previous question for better
        # retrieval — "what about the price?" alone often searches badly
        search_query = build_search_query(question, st.session_state.messages)
        context, sources, is_confident, best_distance = retrieve_context(
            search_query, chunks, metadata, index, st.session_state.confidence_threshold
        )
        ...
        if not is_confident:
            # NEW: confidence-based fallback instead of guessing — with real
            # contact info appended if the owner has set it (deterministic,
            # never AI-generated)
            answer_text = ("I'm not fully sure about this from our current info — let me connect you "
                            "with our support team so they can help directly. 🙏" + format_contact_line(active_business_id))
            ...
            if response_has_unverified_contact_info(answer_text, context):
                answer_text = ("I want to make sure I give you the correct contact details — "
                                ...
                                + format_contact_line(active_business_id))
                placeholder.markdown(f'<div class="assistant-bubble">🧠 {answer_text}</div>', unsafe_allow_html=True)
                save_gap_db(active_business_id, question)
                sources = []
            else:
